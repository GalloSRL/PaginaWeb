...
class ModeloVacancias{
	...
	static public function mdlMostrarConteoPostulantes($tabla, $item, $valor){
		...
		$stmt = Conexion::conectar()->prepare("SELECT conteo_postulante FROM $tabla  WHERE $item = :$item");
	...
	static public function mdlMostrarSumaPostulaciones($tabla, $item, $valor){
		...
		$stmt = Conexion::conectar()->prepare("SELECT SUM(conteo_postulaciones) as 'sumPostulaciones' FROM $tabla");
		$stmt -> execute();
		return $stmt -> fetch(); 
