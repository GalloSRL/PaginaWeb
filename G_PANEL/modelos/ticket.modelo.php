...
class ModeloTickets{
	...
	static public function MdlAñoTickets($tabla, $item, $valor){
		$stmt = Conexion::conectar()->prepare("SELECT YEAR(fecha_solicitud) as year FROM $tabla GROUP BY year");
		$stmt -> execute();
		return $stmt -> fetchAll();	
		$stmt -> close();
		$stmt = null;
	...
	static public function MdlMesTickets($tabla, $item, $valor){
		$stmt = Conexion::conectar()->prepare("SELECT MONTH(fecha_solicitud) as mes FROM $tabla GROUP BY mes");
		$stmt -> execute();
		return $stmt -> fetchAll();	
		$stmt -> close();
		$stmt = null;
	...
	static public function MdlMostrarAñoMesTickets($tabla, $valor1, $valor2){
		$stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla WHERE YEAR(fecha_solicitud) = $valor1 and MONTH(fecha_solicitud) = $valor2");
		$stmt -> execute();
		return $stmt -> fetchAll();	
		$stmt -> close();
		$stmt = null;
	...
	static public function MdlMostrarAñoTickets($tabla, $valor1){
		$stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla WHERE YEAR(fecha_solicitud) = $valor1");
		$stmt -> execute();
		return $stmt -> fetchAll();	
		$stmt -> close();
		$stmt = null;
	...
	static public function mdlIngresarTicket($tabla, $datos){
		$stmt = Conexion::conectar()->prepare("INSERT INTO $tabla(nro_ticket, problema, tipo_problema, prioridad, solicitado_por, asignado_a, id_sucursal) VALUES (:nro_ticket, :problema, :tipo_problema, :prioridad, :solicitado_por, :asignado_a, :id_sucursal)");
	...
	static public function mdlEditarTicket($tabla, $datos){
		...
		$stmt = Conexion::conectar()->prepare("UPDATE $tabla SET problema = :problema, tipo_problema = :tipo_problema, prioridad = :prioridad, solicitado_por = :solicitado_por, asignado_a = :asignado_a, id_sucursal = :id_sucursal WHERE id = :id");
		$stmt->bindParam(":problema", $datos["problema"], PDO::PARAM_STR);
		$stmt->bindParam(":tipo_problema", $datos["tipo_problema"], PDO::PARAM_STR);
		$stmt->bindParam(":prioridad", $datos["prioridad"], PDO::PARAM_STR);
		$stmt->bindParam(":solicitado_por", $datos["solicitado_por"], PDO::PARAM_STR);
	...
	static public function mdlEditarTicketEstado($tabla, $datos){
	
		$stmt = Conexion::conectar()->prepare("UPDATE $tabla SET fecha_fin_trabajo = :fecha_fin_trabajo, tiempo = :tiempo, comentarios = :comentarios, estado = :estado  WHERE id = :id");
