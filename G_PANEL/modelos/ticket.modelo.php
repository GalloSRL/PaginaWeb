...
class ModeloTickets{
	...
	static public function mdlMostrarTickets($tabla, $item, $valor){
		...
		if($item != null){
			...
			END as solicitado ,T0.asignado_a,T0.id_sucursal,T0.comentarios,T0.estado 
			FROM $tabla as T0 LEFT JOIN usuarios T1 ON T0.solicitado_por = T1.id WHERE $item = :$item and T0.borrado = 0 Order By T0.fecha_solicitud desc");
	...
	static public function mdlMostrarTicketsEditar($tabla, $item, $valor){
	...
	static public function mdlMostrarTicketsTop($tabla, $item, $valor){
	...
	static public function mdlIngresarTicket($tabla, $datos){
		...
		$stmt = Conexion::conectar()->prepare("INSERT INTO $tabla(nro_ticket, problema, tipo_problema, prioridad, solicitado_por, asignado_a, id_sucursal, tiempo) VALUES (:nro_ticket, :problema, :tipo_problema, :prioridad, :solicitado_por, :asignado_a, :id_sucursal, :tiempo)");
	...
	/*=============================================
	REGISTRO DE Ticket USer
	=============================================*/
	static public function mdlIngresarTicketUser($tabla, $datos){
		$stmt = Conexion::conectar()->prepare("INSERT INTO $tabla(nro_ticket, problema, prioridad, solicitado_por) VALUES (:nro_ticket, :problema, :prioridad, :solicitado_por)");
		$stmt->bindParam(":nro_ticket", $datos["nro_ticket"], PDO::PARAM_INT);
		$stmt->bindParam(":problema", $datos["problema"], PDO::PARAM_STR);
		$stmt->bindParam(":prioridad", $datos["prioridad"], PDO::PARAM_STR);
		$stmt->bindParam(":solicitado_por", $datos["solicitado_por"], PDO::PARAM_STR);
	...
	EDITAR Ticket User
	=============================================*/
	static public function mdlEditarTicketUser($tabla, $datos){
		$stmt = Conexion::conectar()->prepare("UPDATE $tabla SET problema = :problema, prioridad = :prioridad WHERE id = :id");
		$stmt->bindParam(":problema", $datos["problema"], PDO::PARAM_STR);
		$stmt->bindParam(":prioridad", $datos["prioridad"], PDO::PARAM_STR);
		$stmt->bindParam(":id", $datos["id"], PDO::PARAM_INT);
		if($stmt -> execute()){
